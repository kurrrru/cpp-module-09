...
template<typename Iterator, typename Compare>
void introSortImpl(Iterator begin, Iterator end, std::size_t depthLimit,
    Compare cmp) {
    ...
    Iterator pivot_pos = std::partition(begin, end - 1,
...
void introSort(std::vector<int> &container, Compare cmp) {
    ...
    std::size_t depthLimit = static_cast<std::size_t>(
        std::log(container.size()) / std::log(2));
